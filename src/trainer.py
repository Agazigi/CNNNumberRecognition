...
class Trainer:
    def __init__(self, model, analyzer, train_data, test_data, device, epochs=10, lr=0.01):
        ...
        self.lr = lr # 设置学习率
        self.optimizer = optim.Adam(self.model.parameters(), lr=lr) # 设置优化器，采用 Adam 优化器
    ...
    def train_epochs(self):
        ...
        for epoch in range(self.epochs): # 每一个训练轮数
            ...
            epoch_loss = 0 # 统计损失
            with tqdm(self.train_data, desc=f"第 {epoch + 1} 轮： {epoch + 1}/{self.epochs}", ncols=100) as pbar:  # 使用 tqdm 包裹 train_data, 添加进度条
    ...
    def test_model(self):
        ...
        with tqdm(self.test_data, desc="测试模型", ncols=100) as pbar:
            for X, y in pbar: # 遍历测试数据， X 为输入， y 为标签
                ...
                # F.kl_div 将 y_hat 和 target_dist 相比较，并计算平均值
                total += y.size(0) # 统计样本总数
                correct += (y_hat.argmax(dim=1) == y).sum().item() # 统计预测正确的样本总数
                total_kl_div += kl_div.item() # 将 KL 散度进行累加
                pbar.set_postfix({"acc": correct / total, "kl_div": total_kl_div / (pbar.n + 1)})
        print(f"测试集准确率：{correct / total}") # 测试集准确率
        print(f"测试集平均KL散度：{total_kl_div / len(self.test_data)}") # 测试集平均 KL 散度
